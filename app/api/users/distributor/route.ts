    ...
    const [rows] = await pool.query(
      `SELECT 
          ...
          (
            ...
            WHERE sub_user.distributorId = u.seq AND sub_user.isDeleted = 0 AND sub_user.role =2
          ) AS agencyCount,
          ...
          (
            SELECT COUNT(*) 
            FROM \`User\` AS sub_user 
            WHERE sub_user.distributorId = u.seq AND sub_user.isDeleted = 0 AND sub_user.role =3
          ) AS userCount,

          (
            SELECT COUNT(*) 
            FROM \`Slot\` AS s 
            WHERE s.distributorId = u.seq OR s.userId = u.seq
          ) AS slotCount

      FROM \`User\` AS u
      LEFT JOIN \`User\` AS a ON u.agencyId = a.seq
      LEFT JOIN \`User\` AS d ON u.distributorId = d.seq
      WHERE u.isDeleted = 0 AND u.role =1 `
    );
