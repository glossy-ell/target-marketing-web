...
export default function AgencyConfig(props: Props) {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const fetchUsers = async () => {
      try {
        const res = await fetch('/api/users/agency');
  ...
  return (
    ...
    >
          ...
          >
          <h4 className="font-bold">📅 대행사 관리</h4>
         ...
         <table className="text-xs min-w-full text-center bg-white rounded-lg overflow-hidden border border-gray-200 cursor-default">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-5 py-4 border-b border-gray-300"  style={{ display: currentUser?.role === 0  ? '' : 'none' }}>슬롯 오픈</th>
