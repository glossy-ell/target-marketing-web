...
export default function DistributorConfig(props: Props) {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const fetchUsers = async () => {
      try {
        const res = await fetch('/api/users/distributor');
  ...
  useEffect(() => {
    ...
    users.forEach(user => {
      initialState[user.seq] = user.slotAllow;
    });
    setSlotLocalSwitchState(initialState);
  }, [users]);

  useEffect(() => {
    const initialState: { [key: number]: boolean } = {};
    users.forEach(user => {
      initialState[user.seq] = user.userAllow;
    });
    setUserLocalSwitchState(initialState);
  }, [users]);
  ...
  return (
    <div className="col-span-2 mt-4 mb-4 p-4 border rounded bg-white shadow-sm cursor-pointer"
     ...
     >
          ...
          >
          <h4 className="font-bold">📅 총판 관리</h4>
         ...
         <table className="text-xs min-w-full text-center bg-white rounded-lg overflow-hidden border border-gray-200 cursor-default">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-5 py-4 border-b border-gray-300">슬롯 오픈</th>
              <th className="px-5 py-4 border-b border-gray-300">사용자 추가</th>
              ...
              <th className="px-5 py-4 border-b border-gray-300">전체 대행사</th>
                ...
                <td className="border p-2"   style={{ display: currentUser?.role === 0  ? '' : 'none' }}>
                  <label className="relative inline-block w-10 h-5 cursor-pointer">
                    <input
                      ...
                      checked={slotLocalSwitchState[user.seq] || false}
                      onChange={(e) => {
                        setSlotLocalSwitchState(prev => ({
                          ...prev,
               ...
               <td className="border p-2"   style={{ display: currentUser?.role === 0  ? '' : 'none' }}>
                 <label className="relative inline-block w-10 h-5 cursor-pointer">
                   <input
                     type="checkbox"
                     className="sr-only peer"
                     checked={userLocalSwitchState[user.seq] || false}
                     onChange={(e) => {
                       setUserLocalSwitchState((prev) => ({
                         ...prev,
                         [user.seq]: !prev[user.seq],
                       }));
